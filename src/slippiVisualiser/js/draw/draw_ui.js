...
export function drawOverlay(game, showMatchTimer, showStock,id) {
    ...
    if (showMatchTimer) {
        ...
        layersBg.get(id).ui.fillText(
            sec.length < 5 ? sec[2] + sec[3] : sec[3] + sec[4],
            670,
            70
        );
        layersBg.get(id).ui.strokeText(
    ...
    if (showStock) {
        ...
        for (var i = 0; i < lostStockQueue.length; i++) {
            ...
            if (lostStockQueue[i][2] > 20) {
                ...
                layersBg.get(id).ui.save();
                layersBg.get(id).ui.translate(
                    337 +
                        lostStockQueue[i][0] * 145 +
                        lostStockQueue[i][1] * 30 -
                        2,
                    600 - 2
                );
...
export function drawLoading(id) {
    ...
    var yoff = -50;
    var textScale = 1;
    var textGrad = layersBg.get(id).fg2.createLinearGradient(
        0,
        200 + 70 + yoff,
        0,
        520 - 30 + yoff
