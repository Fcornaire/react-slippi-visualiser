...
export function drawPlayer(g, i, id) {
    var stage = g.stage;
    var p = g.players[i];
    if (p.dead) return;
    ...
    if (
        animations[p.attributes.animID][p.action.name][frame - 1] === undefined
    ) {
        frame = animations[p.attributes.animID][p.action.name].length;
    }
    ...
    if (p.miniView && p.action.name != "SLEEP" && !p.starKO) {
        ...
        if (p.hasNametag) {
            ...
            layersBg.get(id).fg2.font =
                "13px Lucida Console, monaco, monospace";
